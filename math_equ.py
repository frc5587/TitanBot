...
def organize(equation_list: List[str]) -> (Union[sympy.Symbol, None], str):
    """
    Takes the input from the user and discard the "-math" part and then finds the variable that they
    want to solve for nd sets the equation equal to 0, if they aren't solving for a variable then it
    just passes it to the next step, it also removes all spaces
...
def parse_equ(equation: str) -> sympy.Add:
    """
    This parses the string form of the equation and returns something that is more usable to sympy
...
def solve_equ(variable: Union[sympy.Symbol, None],
              equation: Union[sympy.Add, int, float]) -> List[str]:
    """
...
def math_main(user_input: List[str]) -> (List[str], str):
    """
    Manages the solving process, first it organizes it, then it parse the str into an
    equation/expression, finally it solves/simplifies it then returns a list of answers
    ...
    simplified_equ = parse_equ(equation)
